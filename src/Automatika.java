...
public class Automatika extends JFrame implements MouseListener, KeyListener{
	...
	Trace take_trace = null; // take with popupmenu
	...
	/*
      This constructor is use to choose which mode we will use 
	 */
	Automatika(int mode){
		if(mode == 1) // graph none oriente
			this.mode = 1;
		else if(mode == 2) // graph oriente
			this.mode = 2;
	...
	/*
Only active if we have found an object
	 */
	public void init_popup_menu_node()
	{
		System.out.println("init_popup_menu_node");
		init_popup_menu();
	 ...
	 * call by choix_menu, ctrl_y, ctrl_z, mouseListener
	 */
	public void repaint(){
		...
		for(int i = 0; i < trace.size(); i++){
			...
			if(trace.get(i).isLoop()){
				...
				if( d > 80)
					g.drawString(trace.get(i).name, (x1+x2)/2, (y1+y2)/2);
				draw_surface.getGraphics().drawLine(x1, y1, x2, y2); // draw trace between nodes
				if(this.mode == 2){ // THALES
					int ab = d;
					int ac = Math.abs(x2 - x1);
					int bc = Math.abs(y2 - y1);
		...
		for(int i = 0; i < coord.size(); i++){
			...
			g.setColor(Color.black);
			g.drawOval(coord.get(i).x-25, coord.get(i).y-25, 50, 50);
			if(coord.get(i).isEnd()){//if node is end of automaton
				g.drawOval(coord.get(i).x-20, coord.get(i).y-20, 40, 40);			
			}
			if(coord.get(i).isStart()){//if node is start of automaton
				g.drawLine(coord.get(i).x - 25, coord.get(i).y, coord.get(i).x - 100, coord.get(i).y);
				g.drawLine(coord.get(i).x - 25, coord.get(i).y, coord.get(i).x - 50, coord.get(i).y + 15);
				g.drawLine(coord.get(i).x - 25, coord.get(i).y, coord.get(i).x - 50, coord.get(i).y - 15);
			}
...
call by: newTrait
	 ...
	 */
	 ...
	 public void mouseReleased(MouseEvent e)
	 {
		 ...
		 if(menu) // If right after right click: exit menu
		 {
			 popupMenu.removeAll();
			 menu = false;
			 trait_origin = null;
		 ...
		 //move a node if we've moved a node more than 2px
